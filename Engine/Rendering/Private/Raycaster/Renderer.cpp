...
// ---------------------------------------------------------------------------------------------------------------------
void Renderer::Initialize( shared_ptr<const WindowDesc> wd, shared_ptr<IWorldGrid> vg )
{
...
void Renderer::Update( const float )
{
    ...
    if ( m_pVoxelGrid->ReuploadStatus() & EReupload::RequestStaging )
    {
        ...
        m_pMemory->UploadOnStreamBuffer(
            ...
            m_pPipeline->GetUniformUploadDescriptor( m_StageVoxelBuffer, VoxelPipeline::EShaderResource::VoxelGrid ) );
